...
- 🏠 **只有自己的領地才便宜**：低費率只適用於你「擁有」的領地；別人的領地一律以荒野費率計費。
...
- 🗺️ **可設定飛行地點**：可依世界限制，並分別開關領地內 / 領地外的付費飛行。
...
### v1.3.1
- 在 `config.yml` 加上專案 GitHub 連結。

### v1.3.0
- 別人的領地改以**荒野費率**計費——便宜費率只適用於你「擁有」的領地，並新增「別人領地」專屬提示。

### v1.2.0
- 落地顯示**飛行明細**（飛多久 + 共花多少），可用 `/afly summary` 個人開關。
- **`/afly notify`** 讓地主自行開關「有人在你領地飛行」的通知（每人獨立）。
- **`/afly lang <語言>`** 可在遊戲內切換伺服器語言。
- 個人偏好持久化於 `playerdata.yml`。

### v1.1.0
- `config.yml` 改為預設**英文**；切換 `language` 並 `/afly reload` 後，設定檔註解也會本地化（數值保留）。
...
*作者：avery11111102、Avery11111101*
